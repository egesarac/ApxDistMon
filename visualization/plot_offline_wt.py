#!/usr/bin/env python3
"""Generate the offline water-tank case-study figure."""
...
DEFAULT_OUTPUT = BENCHMARK_ROOT / "results/figures/speedup_offline_wt.pdf"
...
GENERATED_FILES = (
    ("ADM", "water_tanks.txt"),
    ("ADM-F", "water_tanks_adm_f.txt"),
    ("ADM-FR", "water_tanks_adm_fr.txt"),
    ("ADM-C", "water_tanks_adm_c.txt"),
    ("ADM-CR", "water_tanks_adm_cr.txt"),
)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    source.add_argument(
        ...
        type=Path,
        help=(
            "generated result root containing matching approximate and "
            f"exact run directories (default: {DEFAULT_GENERATED_INPUT})"
        ),
    )
    parser.add_argument(
        "--out",
...
def load_results(input_path: Path) -> dict[int, dict[str, list[float]]]:
    ...
    if not input_path.is_file():
        raise FileNotFoundError(f"missing water-tank results: {input_path}")
...
def load_generated_results(
    ...
    if official_approximate.is_dir():
        run_paths = [(
            ...
            input_path / "exact/case_studies/water_tanks.txt",
        )]
    else:
        run_paths = [
            (
                ...
                "exact/case_studies/water_tanks.txt",
            )
    ...
    if not run_paths:
        raise FileNotFoundError(f"no generated water-tank runs in {input_path}")
...
def plot_offline_wt(input_path: Path, output_path: Path) -> None:
    plot_results(
...
def plot_generated_offline_wt(input_path: Path, output_path: Path) -> None:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.input is not None:
        plot_offline_wt(args.input, args.out)
    else:
        plot_generated_offline_wt(
