...
void run(const Options &options)
{
    ...
    const fs::path outputPath = options.outputDirectory / "onlineIncr_since.txt";
                ...
                for (int c = firstSample; c <= lastSample; ++c)
                {
                    ...
                    auto runMonitor = [&]()
                    {
                        ...
                        while (dPrevious < d)
                        {
                            ...
                            {
                                const auto [since0, since1] =
                                    bitsetLastBits(retainedRoot.back());
                                const auto [lhs0, lhs1] =
                                    bitsetLastBits(retainedP.back());
                                carry0 = since0 || lhs0;
                                carry1 = since1 && lhs1;
                            }
                            auto newRoot = bitsetSinceNonStrict(
                                newP,
                                newQ,
                                0,
                                newP.size(),
