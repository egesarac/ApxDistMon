...
void run(const Options &options)
{
    ...
    const long long H_PHI = 2000;

    fs::create_directories(options.outputDirectory);
    const fs::path outputPath = options.outputDirectory / "onlineIncr_response02.txt";
            ...
            for (auto CHUNK_SZ : CHUNK)
            {
                ...
                for (int c = firstSample; c <= lastSample; ++c)
                {
                    ...
                    auto runMonitor = [&]()
                    {
                        ...
                        vector<vector<bitset<SIZE>>> retainedOnce;
                        vector<vector<bitset<SIZE>>> retainedImplication;
                        ...
                        while (dPrevious < d)
                        {
                            ...
                            auto combinedOnce = bitsetBoundedEventuallyPast(
                                ...
                                historyCount,
                                combinedQ.size());
                            vector<vector<bitset<SIZE>>> newOnce(
                                combinedOnce.begin() + historyCount,
                                combinedOnce.end());

                            auto newImplication = bitsetDisjunction(
                                bitsetNegation(newP),
                                newOnce);
                            ...
                            if (retainedP.size() != segmentCount ||
                                ...
                                retainedOnce.size() != segmentCount ||
                                retainedImplication.size() != segmentCount ||
