...
void run(const Options &options)
{
    ...
    const fs::path outputPath = options.outputDirectory / "onlineIncr_ad.txt";
            ...
            for (auto CHUNK_SZ : CHUNK)
            {
                ...
                for (int c = firstSample; c <= lastSample; ++c)
                {
                    ...
                    auto runMonitor = [&]()
                    {
                        ...
                        vector<vector<bitset<SIZE>>> retainedDisjunction;
                        ...
                        while (dPrevious < d)
                        {
                            ...
                            auto newDisjunction = bitsetDisjunction(newP, newQ);
                            ...
                            retainedDisjunction.insert(
                                retainedDisjunction.end(),
                                newDisjunction.begin(),
                                newDisjunction.end());
                            ...
                            if (retainedP.size() != segmentCount ||
                                ...
                                retainedDisjunction.size() != segmentCount ||
                            ...
                            if (keep > 0)
                            {
                                ...
                                retainedDisjunction.erase(
                                    retainedDisjunction.begin(),
                                    retainedDisjunction.begin() + keep);
