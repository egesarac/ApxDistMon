...
void run(const Options &options)
{
    ...
    const fs::path outputPath = options.outputDirectory / "onlineNaive_ed.txt";
                ...
                for (int c = firstSample; c <= lastSample; ++c)
                {
                    ...
                    auto runMonitor = [&]()
                    {
                        ...
                        while (dPrevious < d)
                        {
                            ...
                            auto disjunction = bitsetDisjunction(aps[0], aps[1]);
                            auto root = bitsetEventuallyPast(disjunction);
