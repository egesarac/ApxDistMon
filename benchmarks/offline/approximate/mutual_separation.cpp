...
struct Options
{
    ...
    fs::path dataDirectory = "data/case_studies/uav";
...
void printUsage(const char *program)
{
    std::cout
        ...
        << "  --agents <2|3|4>             Restrict the drone count\n"
        << "  --epsilon-multiplier <n>     Restrict epsilon to n * 50 ms\n"
        << "  --variant <name|all>         adm, adm-f, adm-fr (default: adm)\n"
        << "  --repeat <count>             Timed repetitions (default: 1)\n"
        << "  --data-dir <path>            UAV dataset directory\n"
...
std::vector<Variant> selectVariants(const std::string &selection)
{
    ...
    const Variant variant = parseVariant(selection);
    if (isCoarse(variant))
    {
        throw std::runtime_error(
            "coarse variants do not apply to mutual separation");
    }
    return {variant};
}
...
fs::path outputPath(const Options &options, Variant variant)
{
    ...
    {
        return options.outputDirectory / "mutual_separation.txt";
    }
    return options.outputDirectory /
           ("mutual_separation_" + variantFileSuffix(variant) + ".txt");
}

void runVariant(
    Variant variant,
    const Options &options,
    const PointSignals &allSignals)
{
    ...
    const std::vector<int> epsilonMultipliers = selectedValues(
        {1, 2, 3, 4, 5}, options.epsilonMultiplier);
    ...
    {
        const PointSignals signals(allSignals.begin(), allSignals.begin() + agents);
...
int main(int argc, char **argv)
{
    try
    {
        ...
        PointSignals signals;
        signals.reserve(maximumAgents);
        for (int agent = 0; agent < maximumAgents; ++agent)
        {
            PointSignal signal = loadPointSignal(
                options.dataDirectory / ("s1_uav_" + std::to_string(agent)));
