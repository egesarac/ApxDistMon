...
`bitsetSinceNonStrictLegacy` remain independent valid-input differential
oracles. The Since hook routes its frozen reversal, carry, and allocation
adapter through the frozen pre-refactor scalar non-strict Until implementation,
so neither the optimized past transformation nor the shared checked Until
segment kernel is part of its oracle path.
...
- all 16,384 ordered one-segment Since cases over the same language space and
  carries;
- all 262,144 two-segment Since comparisons formed from `16^4` operand segment
  tuples under all four incoming carries;
...
The complete Release CTest run passed 7/7. The expanded past differential
targets passed under ASan/UBSan as part of the focused 4/4 sanitizer run.
Strict Since remains disabled under `#if 0` and is absent from the maintained
target and dispatcher lists.
...
Across constant, ambiguous, and rich fixtures, non-strict Since improved by
2.43--2.87x.
